    ...
    if (!ci.isInstanceOf("java.lang.Cloneable")) {
    	DynamicDependency exDep = null;
  		ObjectProperty objPropAttr = 
  				(ObjectProperty) env.getHeap().get(objref).getObjectAttr();
  		if (objPropAttr != null) {
  			exDep = DynDepBuilder.newBuilder().appendDataDependency(objPropAttr.getTypeDependency())
  					.setControlDependency(getInvocationCondition(env)).build();
  		}
      env.throwException("java.lang.CloneNotSupportedException",
          ci.getName() + " does not implement java.lang.Cloneable.", exDep);
      return MJIEnv.NULL;  
      ...
      DynamicDependency resultDep = DynDepBuilder.newBuilder()
      		.appendDataDependency(argAttrs[0])
      		.setControlDependency(getInvocationCondition(env)).build();
