...
public class JPF_java_lang_Class extends NativePeer {
  ...
  public int getPrimitiveClass__Ljava_lang_String_2__Ljava_lang_Class_2 (MJIEnv env,
    ...
    ClassLoaderInfo scli = env.getSystemClassLoaderInfo(); 
    String primClsName = env.getStringObject(stringRef); 
  ...
  public int getDeclaredConstructor___3Ljava_lang_Class_2__Ljava_lang_reflect_Constructor_2 (MJIEnv env,
    ...
    if (argAttrs != null) {
    	for (Object argAttr : argAttrs) {
    		deps.add((DynamicDependency) argAttr);
    	}
    }
    if (argTypesRef != MJIEnv.NULL) {
	    for (int i = 0; i < env.getArrayLength(argTypesRef); ++i) {
	    	deps.add((DynamicDependency) env.getElementAttr(argTypesRef, i));
	    }
    }
    DynamicDependency resultDep = DynDepBuilder.newBuilder()
    		.appendDataDependency(deps)
  ...
  private void addDeclaredMethodsRec (boolean includeSuperClasses, HashMap<String,MethodInfo>methods, ClassInfo ci){
    ...
    for (String ifcName : ci.getDirectInterfaceNames()){
      ClassInfo ici = cl.getResolvedClassInfo(ifcName); 
