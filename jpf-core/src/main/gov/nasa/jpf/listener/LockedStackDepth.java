...
public class LockedStackDepth extends ListenerAdapter
{
   ...
   public void objectLocked(VM vm, ThreadInfo thread, ElementInfo ei)
   {
      ...
      if (m_state.containsKey(makeKey(lock)))               
   ...
   private void switchTo(Operation next)
   {
      ...
      for (operation = next; operation != null; operation = operation.getParent())  
      {
         m_index.put(operation, m_apply.size());  
         m_apply.add(operation);
      }
      
      index = null;
      
      for (operation = m_current; operation != null; operation = operation.getParent())  
      {
