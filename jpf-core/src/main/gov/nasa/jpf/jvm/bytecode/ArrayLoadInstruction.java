  ...
  public Instruction execute (ThreadInfo ti) {
    ...
    if (!eiArray.checkArrayBounds(index)) {
    	...
    	ArrayProperty arrayPropAttr = 
    			(ArrayProperty) eiArray.getObjectAttr();
    	if (arrayPropAttr != null) {
    		arraySizeDep = arrayPropAttr.getArrayLengthDependency();
    	}
    	DynamicDependency exDep = DynDepBuilder.newBuilder()
    			.appendDataDependency(
    					arrayOperandAttr, indexOperandAttr, arraySizeDep, getInstructionDepSource(ti))
    			.setControlDependency(getControlDependencyCondition(ti)).build();
    ...
    DynamicDependency resultDependences = DynDepBuilder.newBuilder()
    		.appendDataDependency(indexOperandAttr, arrayOperandAttr, 
    				elementAttr, getInstructionDepSource(ti))
    		.setControlDependency(getControlDependencyCondition(ti)).build();
