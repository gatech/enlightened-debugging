...
public class PutHelper {
  ...
  public static long setField (ThreadInfo ti, StackFrame frame, 
  		ElementInfo eiFieldOwner, FieldInfo fi, Object... extraDependences){
    int fieldSize = fi.getStorageSize();
    Object valAttr = null;
    long ret = 0;
    ...
    Object[] deps = new Object[extraDependences.length + 1];
    System.arraycopy(extraDependences, 0, deps, 0, extraDependences.length);
    deps[deps.length - 1] = valAttr;
    DynamicDependency resultDependences = DynDepBuilder.newBuilder()
    		.appendDataDependency(deps).build();
    eiFieldOwner.setFieldAttr(fi, resultDependences);
    return ret;
  }
}
