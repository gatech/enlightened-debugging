...
public class LDIV extends DependencyTrackingInstruction implements JVMInstruction {

  @Override
  public Instruction execute (ThreadInfo ti) {
    StackFrame frame = ti.getModifiableTopFrame();
    
    Object v1Attr = frame.getLongOperandAttr();
    long v1 = frame.popLong();
    Object v2Attr = frame.getLongOperandAttr();
    long v2 = frame.popLong();

    if (v1 == 0) {
    	DynamicDependency exDep = DynDepBuilder.newBuilder()
    			.appendDataDependency(v1Attr, getInstructionDepSource(ti))
    			.setControlDependency(getControlDependencyCondition(ti)).build();
      return ti.createAndThrowException(
      		"java.lang.ArithmeticException", "long division by zero", exDep);
    }
