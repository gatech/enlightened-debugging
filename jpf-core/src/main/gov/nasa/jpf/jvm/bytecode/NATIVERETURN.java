...
public class NATIVERETURN extends JVMReturnInstruction {
  ...
  public Instruction execute (ThreadInfo ti) {
    ...
    if (!frame.isDependencyTracked()) {
	    NativeMethodDependencyHandler depHandler = 
	    		new NativeMethodDependencyHandler(frame, ti);
	    if (depHandler.canHandle()) {
	    	List<DynamicDependency> argDeps = depHandler.extractArgDependencies();
	    	argDeps.add(getInstructionDepSource(ti));
	    	DynamicDependency resultDep = DynDepBuilder.newBuilder()
	    			.appendDataDependency(argDeps)
	    			.setControlDependency(getControlDependencyCondition(ti)).build();
	    	depHandler.setReturnValueDependency(resultDep);
	    }
    }
    
    getAndSaveReturnValue(frame);

    StackFrame callerFrame = ti.popAndGetModifiableTopFrame();


    callerFrame.removeArguments(mi);
    pushReturnValue(callerFrame);
    ...
    return callerFrame.getPC().getNext();
  }
