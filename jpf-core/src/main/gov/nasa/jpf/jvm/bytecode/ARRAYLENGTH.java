...
public class ARRAYLENGTH extends DependencyTrackingInstruction implements JVMInstruction {
  ...
  public Instruction execute (ThreadInfo ti) {
    ...
    if (arrayRef == MJIEnv.NULL){
    	DynamicDependency exDep = DynDepBuilder.newBuilder()
    			.appendDataDependency(arrayRefAttr, getInstructionDepSource(ti))
    			.setControlDependency(getControlDependencyCondition(ti)).build();
      return ti.createAndThrowException("java.lang.NullPointerException",
                                        "array length of null object", exDep);
    }

    ElementInfo ei = ti.getElementInfo(arrayRef);
    frame.push(ei.arrayLength(), false);
    
    DynamicDependency arrayLengthDependences = null;
    ArrayProperty arrayDependences = (ArrayProperty) ei.getObjectAttr();
    if (arrayDependences != null) {
    	arrayLengthDependences = arrayDependences.getArrayLengthDependency();
    }
    DynamicDependency resultDependences = DynDepBuilder.newBuilder()
    		.appendDataDependency(arrayRefAttr, arrayLengthDependences, getInstructionDepSource(ti))
    		.setControlDependency(getControlDependencyCondition(ti)).build();
