...
public class java_textTest extends TestJPF {
  private final static String[] JPF_ARGS = {"+nhandler.delegateUnhandledNative = true"};
  ...
  public void convertSimpleDateFormatTest() {
    if(verifyNoPropertyViolation(JPF_ARGS)) {
      ...
      // Note(xiangyuli): Not sure what the failure is on the author's machine, but
      // on my machine, these two assertions run just fine except that the expected
      // value themselves are incorrect.
      ...
      assertEquals(format.format(calendar.getTime()),
          retFormats[0].format(calendar.getTime()));
      SimpleDateFormat format2 = new SimpleDateFormat(
      		JPF_converter_specific_java_textTest.SDF_PATTERN2);
      assertEquals(format2.format(calendar.getTime()),
          retFormats[1].format(calendar.getTime()));
    }
  }
}
